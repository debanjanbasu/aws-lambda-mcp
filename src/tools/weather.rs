...
pub async fn get_weather(request: WeatherRequest, http_client: &dyn HttpClient) -> Result<WeatherResponse, AppError> {
    info!(
    ...
    let (latitude, longitude, timezone) = geocode_location(&request.location, http_client).await?;

    // Fetch weather data
    let weather_data = fetch_weather_data(latitude, longitude, &timezone, http_client).await?;
...
async fn geocode_location(location: &str, http_client: &dyn HttpClient) -> Result<(f64, f64, String), AppError> {
...
async fn fetch_weather_data(latitude: f64, longitude: f64, timezone: &str, http_client: &dyn HttpClient) -> Result<WeatherResponse, AppError> {
    ...
    let open_meteo_response: OpenMeteoResponse = http_client
        .get_json_value(&weather_url)
        .await
        .map_err(|e| AppError::WeatherApiError(format!("Failed to fetch weather forecast: {e}")))?
        .try_into()
...
impl TryFrom<serde_json::Value> for OpenMeteoResponse {
