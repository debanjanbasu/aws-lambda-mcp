...
mod tests {
    use super::*;
    use lambda_runtime::ClientContext;
    use serde_json::json;
    ...
    fn test_extract_tool_name_from_client_context() {
        ...
        custom.insert(
            "bedrockAgentCoreToolName".to_string(),
            "gateway-123___get_weather".to_string(),
        );
        let client_context = ClientContext {
            custom,
            ..Default::default()
        };
        let context = lambda_runtime::Context {
    ...
    async fn test_route_tool_invalid_input() {
        let event_payload = json!({"location": 123}); // invalid, location should be string
